...
!function($){
	...
	Typeahead.prototype = {
		...
		select: function () {
			var data = this.$menu.find('.active').data('item')
            var value = data
            if (this.ajax.displayField) {
                value = data[this.ajax.displayField]
            }
			this.$element.val(this.updater(value)).change()
		...
		ajaxSource: function (data) {
			...
			items = $.grep(data, function (item) {
                ...
                if (that.ajax.displayField) {
                    value = value[that.ajax.displayField]
				}
				if (that.matcher(value)) return item
			})
		...
		sorter: function (items) {
			...
			while (item = items.shift()) {
                var value = item
				if (this.ajax.displayField) {
                    value = item[this.ajax.displayField]
				}
				if (!value.toLowerCase().indexOf(this.query.toLowerCase())) beginswith.push(item)
				else if (~value.indexOf(this.query)) caseSensitive.push(item)
		...
		render: function (items) {
			...
			items = $(items).map(function (i, item) {
                var value = item
				if (that.ajax.displayField) {
                    value = item[that.ajax.displayField]
				}
